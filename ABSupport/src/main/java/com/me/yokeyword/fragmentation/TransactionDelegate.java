...
import com.me.yokeyword.fragmentation.exception.AfterSaveStateTransactionWarning;
import com.me.yokeyword.fragmentation.helper.internal.ResultRecord;
import com.me.yokeyword.fragmentation.helper.internal.TransactionRecord;
import com.me.yokeyword.fragmentation.queue.Action;
import com.me.yokeyword.fragmentation.queue.ActionQueue;
...
class TransactionDelegate {
    ...
    void loadMultipleRootTransaction(final FragmentManager fm, final int containerId, final int showPosition, final List<? extends ISupportFragment> tos) {
        enqueue(fm, new Action(Action.ACTION_LOAD) {
            @Override
            public void run() {
                FragmentTransaction ft = fm.beginTransaction();
                for (int i = 0; i < tos.size(); i++) {
                    Fragment to = (Fragment) tos.get(i);

                    Bundle args = getArguments(to);
                    args.putInt(FRAGMENTATION_ARG_ROOT_STATUS, SupportFragmentDelegate.STATUS_ROOT_ANIM_DISABLE);
                    bindContainerId(containerId, tos.get(i));
